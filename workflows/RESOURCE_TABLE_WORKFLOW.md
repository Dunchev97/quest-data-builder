...
- Во всех строках типов использовать `sl`; `temp`, `temp_01`, `temp_02` не использовать.
- `id` оставлять пустым.
- Значения полей, объявленных как `int`, хранить числами, а не текстовыми строками. Это относится в том числе к вероятностям GR, параметрам post action, пакетам и рецептам.
...
4. Найти выбранные интерактивные объекты campaign в `campaigns/<campaign_id>/interactive_objects.json`.
5. Взять `_R_` ресурсы выбранных интерактивных объектов по кругу: 1-й craft = объекты 1+2, 2-й craft = 3+4, если 4-го нет = 3+1, дальше продолжать тем же правилом.
6. Если выбранных интерактивных объектов нет, не блокировать сборку: использовать старый fallback 1/2 ингредиентов и записать это как технический риск только при необходимости.
...
Не дублировать 1-й и 2-й ингредиенты в 3-й и 4-й слот, если для campaign уже зафиксированы интерактивные объекты.
...
- в `stage6_review.xlsx` лист `РЕСУРСЫ` имеет `freeze_panes=A76`.
